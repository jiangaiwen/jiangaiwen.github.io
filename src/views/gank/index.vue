<template>
    <div class="gank">
        <div class="container">
            <p class="theme" @click="changeTheme">{{theme=='light'?'dark':'light'}}</p>
            <div class="gank-grid">
...
export default {
    ...
    methods: {
        getData(){
            this.axios.get('/gank/api/v2/data/category/Girl/type/Girl/page/'+this.page+'/count/10').then((res) => {
                if(res.data.data){
        ...
        getHotGril(){
            this.axios.get('/gank/api/v2/hot/views/category/Girl/count/3').then((res) => {
                if(res.data.data){
